 ...
 * @author FASIH
 */
public class L_stokMakanan extends javax.swing.JFrame {
private koneksi db = new koneksi();
int baris;
    /**
     * Creates new form L_stok_makanan
     */
    public L_stokMakanan() {
        initComponents();
     ...
     public void TableAddRow(String keyword){
        ...
        tbl.addColumn("Harga");
        tbl.addColumn("Stok Makanana");
        jTable1.setModel(tbl);
        try {
            if (keyword != "") {
                String sql = "SELECT *,foods.name as nama_makanan, category.name as nama_category FROM foods JOIN category ON foods.categoryID = category.categoryID WHERE foods.foodID  LIKE '%"+keyword+"%' OR foods.name LIKE '%"+keyword+"%' OR category.name LIKE '%"+keyword+"%' ORDER BY foods.food_stock ASC";  
                ...
                String sql = "SELECT *,foods.name as nama_makanan, category.name as nama_category FROM foods JOIN category ON foods.categoryID = category.categoryID ORDER BY foods.food_stock ASC";   
                    PreparedStatement ps = db.koneksidb().prepareStatement(sql);
                    ResultSet rs = ps.executeQuery();
                    int no = 1;
                    while (rs.next()) {                
    ...
    private void btn_cetak_laporanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_cetak_laporanActionPerformed
        try {
            String sql = "SELECT COUNT(foods.foodID) as stok_makanan from foods JOIN category ON foods.categoryID=category.categoryID";
            PreparedStatement ps = db.koneksidb().prepareStatement(sql);
            ResultSet rs  = ps.executeQuery();
            rs.next();
            ...
            // Menyusun jalur file laporan untuk Lap_stokMakanan.jasper dengan pemisah direktori yang benar
            String path = appDirectory + File.separator + "src" + File.separator + "kopikuy" + File.separator + "Lap_stokMakanan.jasper";

            Map parameter = new HashMap();
            parameter.put("row",rs.getInt("stok_makanan"));
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(L_stokMakanan.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new L_stokMakanan().setVisible(true);
            }
        });
    }
